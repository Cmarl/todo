...
function init(){
  ...
  root = new Firebase('https://todo-clyde.firebaseio.com/');
...
function taskAdded(snapshot){
  ...
  var checked = task.isComplete ? 'checked' : '';
  var tr = '<tr class="' + checked + '"data-key="'+key+'"><td><button class="delete">X</button></td><td><input type="checkbox"'+ checked+ '></td><td>' + task.title + '</td><td>' + moment(task.dueDate).format('YYYY-MM-DD') + '</td><td>' + task.priority + '</td><td>' + moment(task.createdAt).format('YYYY-MM-DD') + '</td></tr>';
